...
import com.aiforest.tmmes.center.auth.service.*;
...
public class AuthServiceImpl implements AuthService {
    ...
    public AuthUser login(Login login) {
        ...
        //2.2 resources
        Set<com.aiforest.tmmes.common.model.Resource> resourceSet = new HashSet<>();
        for (Role role : roles) {
            List<com.aiforest.tmmes.common.model.Resource> resources = roleResourceBindService.listResourceByRoleId(role.getId());
            resourceSet.addAll(resources);
        }
        if (CollUtil.isEmpty(resourceSet)) {
            throw new ServiceException("请先为用户{}分配权限", login.getName());
        }
        Set<String> resourceCodeSet = resourceSet.stream().map(com.aiforest.tmmes.common.model.Resource::getResourceCode).collect(Collectors.toSet());
