...
import com.aiforest.tmmes.center.manager.mapper.PointAttributeConfigMapper;
import com.aiforest.tmmes.center.manager.mapper.PointAttributeMapper;
...
public class PointAttributeConfigServiceImpl implements PointAttributeConfigService {
    ...
    @Override
    public void updateOrAdd(PointAttributeConfig entityDO) {
        try {
            PointAttributeConfig pointAttributeConfig = selectByAttributeIdAndDeviceIdAndPointId(entityDO.getPointAttributeId(), entityDO.getDeviceId(), entityDO.getPointId());
            pointAttributeConfig.setOperateTime(null);
            pointAttributeConfig.setConfigValue(entityDO.getConfigValue());
            if (pointAttributeConfigMapper.updateById(pointAttributeConfig) < 1) {
                throw new UpdateException("The point attribute config update failed");
            }
            ...
            if (pointAttributeConfigMapper.insert(entityDO) < 1) {
                PointAttribute pointAttribute = pointAttributeMapper.selectById(entityDO.getPointAttributeId());
                throw new AddException("The point attribute config {} add failed", pointAttribute.getDisplayName());
            }
